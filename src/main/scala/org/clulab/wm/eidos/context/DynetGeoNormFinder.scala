...
object DynetGeoNormFinder {

  def fromConfig(config: Config): DynetGeoNormFinder = {
...
class DynetGeoNormFinder(extractor: GeoLocationExtractor, normalizer: GeoLocationNormalizer) extends Finder {

  def getGeoPhraseIDs(odinMentions: Seq[Mention], sentences: Array[Sentence]): Array[Seq[GeoPhraseID]] =
    DynetGeoNormFinder.getGeoPhraseIDs(odinMentions, sentences)
