...
import org.clulab.wm.eidos.context.{GeoPhraseID, TimEx, TimeNormFinder, TimeStep}
...
//  }
  ...
  def combineStartEndTimes(mentions: Seq[Mention]): Seq[Mention] = {
    ...
    for (m <- relevant) {
      ...
      for (timex <- timexs) {
        for (d <- timex.intervals) {
          println("timex:\t" + d.startDate)
          mentionStartDate.append(d.startDate)
        }
      }
      ...
      val newArg = m.arguments ++ Map("time" -> Seq(newTextBoundMention))
      val newMention = copyWithNewArgs(m, newArg, Some("timeMerged"))
