...
object Timer {
  ...
  def time[R](description: String)(block: => R): R = {
    val t0 = System.currentTimeMillis()
    logger.info(s"\tStart\t$description\t$t0\tms")

    val result: R = block // call-by-name

    val t1 = System.currentTimeMillis()
    logger.info(s"\tStop\t$description\t$t1\tms")

    val diff = t1 - t0
    logger.info(s"\tDiff\t$description\t$diff\tms")
    logger.info(s"\tTime\t$description\t${diffToString(diff)}")
    result
  }
}
