...
object EmptyEidosShell extends App {
  ...
  val mainMenuItems = Seq(
    ...
    new MainMenuItem(":reload", "reload grammar", (_: Menu, _: String) => {
      println("Reloading..."); true
    }),
    new ExitMenuItem(":exit", "exit system")
  )
  val defaultMenuItem = new DefaultMenuItem((_: Menu, text: String) => {
    println(s"""Processing "$text"..."""); true
  })
