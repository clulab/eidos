...
object OntologyMapper {
  ...
  def mkMWEmbedding(s: String, reader: EidosSystem, contentOnly: Boolean = false): Array[Float] = {
    ...
    reader.config.ontologyHandler.wordToVec.makeCompositeVector(selectWords(words, contentOnly, reader.config.proc))
  }
  ...
  def mapIndicators(reader: EidosSystem, outputFile: String, topN: Int): Unit = {
    val grounders: Seq[EidosOntologyGrounder] = reader.config.ontologyHandler.grounders
  ...
  def mapOntologies(reader: EidosSystem, bbnPath: String, sofiaPath: String, providedOntology: Option[String], providedOntName: String = "ProvidedOntology",
    ...
    val w2v = reader.config.ontologyHandler.wordToVec
//    val config = reader.config

    // Load
    val eidosConceptEmbeddings = if (providedOntology.nonEmpty) {
      val grounder = OntologyHandler.mkDomainOntologyFromYaml(providedOntName, providedOntology.get, proc, new Canonicalizer(reader.config.stopwordManager))
      grounder match {
      ...
      reader.config.ontologyHandler.grounders.head.conceptEmbeddings
    }
