...
object ExtractFromFile extends App {
  ...
  (FileUtils.printWriterFromFile(s"$outputFile")).autoClose { pw =>
    ...
    for (filename <- files) {
      ...
      val eidosMentions = annotatedDoc.eidosMentions.filter(em => ieSystem.components.stopwordManagerOpt.get.isCAGRelevant(em.odinMention, cagEdgeMentions, cagEdgeArguments))
