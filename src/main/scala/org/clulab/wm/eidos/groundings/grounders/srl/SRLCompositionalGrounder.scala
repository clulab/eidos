...
class SRLCompositionalGrounder(name: String, domainOntology: DomainOntology, w2v: EidosWordToVec, canonicalizer: Canonicalizer, tokenizer: EidosTokenizer)
  ...
  def groundSentenceSpan(
    ...
    def findExactPredicateGroundingAndRanges(mentionWords: Array[String], mentionLemmas: Array[String], mentionIndexes: Seq[Int], validIndexes: Seq[Int]):
        Seq[(PredicateGrounding, Seq[Int], String)] = {
      SRLCompositionalGrounder.processOrConceptBranches.flatMap { branch =>
        val exactMatches = exactMatchesForPreds(mentionWords, mentionLemmas, conceptEmbeddingsMap(branch), mentionIndexes, validIndexes)
    ...
    def groundWithPredicates(sentenceHelper: SentenceHelper): Seq[PredicateGrounding] = {
      val predicateGroundings = {
        ...
        val exactPredicateGroundingsAndIndexesAndBranches = findExactPredicateGroundingAndRanges(exactWords, exactLemmas, exactIndexes, sentenceHelper.validTokenIndexes)
