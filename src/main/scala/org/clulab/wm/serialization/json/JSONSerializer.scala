...
//  import org.clulab.odin.serialization.json._
  ...
  def jsonAST(mentions: Seq[Mention]): JValue = {
    ...
    val mentionList = JArray(mentions.map(m => MentionOps.jsonAST(m)).toList)
    ...
    mjson \ "type" match {
      ...
      case JString(WMCrossSentenceMention.string) =>
        ...
        new CrossSentenceMention(
          labels,
          anchor = args.values.flatten.find(MentionOps.id(_) == anchorID).get,
          neighbor = args.values.flatten.find(MentionOps.id(_) == neighborID).get,
          mkArgumentsFromJsonAST(mjson \ "arguments"),
          document,
          keep,
          foundBy,
