...
class TestCagP6 extends EnglishTest {
  ...
  { // S1
    ...
    affectEventTest should "have correct edges 4" taggedAs(Becky) in {
  ...
  { // S2
   ...
   val insecurity   = NodeSpec("levels of food insecurity", Inc("high"), Quant("high", "unprecedented"))
    ...
    val access       = NodeSpec("poor access to services", Dec("poor"), Quant("poor")) // yes
    ...
    brokenSyntaxTest should "have correct edges 1" taggedAs(Becky) in { // good
      tester.test(EdgeSpec(insecurity, Causal, malnutrition)) should be (successful)
    }
    brokenSyntaxTest should "have correct edges 2" taggedAs(Becky) in { // good
    ...
    brokenSyntaxTest should "have correct edges 3" taggedAs(Becky) in {
      tester.test(EdgeSpec(access, Causal, malnutrition)) should be (successful)
    }
    brokenSyntaxTest should "have correct edges 4" taggedAs(Becky) in { // good
      tester.test(EdgeSpec(morbidity, Causal, malnutrition)) should be (successful)
    }
    brokenSyntaxTest should "have correct edges 5" taggedAs(Becky) in { // good
      tester.test(EdgeSpec(diet, Causal, malnutrition)) should be (successful)
    }
    brokenSyntaxTest should "have correct edges 6" taggedAs(Becky) in {
