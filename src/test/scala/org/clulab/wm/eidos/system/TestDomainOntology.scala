...
//    ontologyNodes.foreach(println)
  ...
  it should "load and not have duplicates" in {
    hasDuplicates(UNOntology("un", "/org/clulab/wm/eidos/english/ontologies/un_ontology.yml", "", proc, filter)) should be (false)
  }

  behavior of "fao ontology"
  it should "load and not have duplicates" in {
    hasDuplicates(FAOOntology("fao", "/org/clulab/wm/eidos/english/ontologies/fao_variable_ontology.yml", "", proc, filter)) should be (false)
  }

  behavior of "wdi ontology"
  it should "load and not have duplicates" in {
    hasDuplicates(WDIOntology("wdi", "/org/clulab/wm/eidos/english/ontologies/wdi_ontology.yml", "", proc, filter)) should be (false)
  }
