...
class TestSentenceClassifier extends Test {
  ...
  class TestableSentenceClassifier extends SentenceClassifier {
  ...
  ignore should "work integrated" in {
    val sentenceClassifier = new TestableSentenceClassifier()
    val eidosEnglishProcessor = new EidosEnglishProcessor(sentenceClassifier, Language.ENGLISH, 100)
