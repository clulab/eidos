...
object CAG {
  ...
  val p1raw = Array(p1s1, p1s2, p1s3, p1s4).mkString(SINGLE_SENTENCE_SEPARATOR)
  val p1 = p1raw.replaceAll("\u2013", "-") // TODO: make sure tokenizer does this
  ...
  val p2raw = Array(Array(p2s1, p2s2).mkString(SINGLE_SENTENCE_SEPARATOR), p2s3, p2s4, p2s5).mkString(DOUBLE_SENTENCE_SEPARATOR)
  val p2 = p2raw.replaceAll("\u2019", "'")
  ...
  val p4raw = Array(p4s1, p4s2, p4s3, p4s4).mkString(SINGLE_SENTENCE_SEPARATOR)
  val p4 = p4raw.replaceAll("\u2019", "'")
