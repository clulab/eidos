...
./bin/rest-producer-loop-app2 "${DATA_DIR}/output" "${DATA_DIR}" "${DATA_DIR}/output/done" &
sleep 10
./bin/rest-consumer-loop-app2 "${DATA_DIR}/input/kafka" "${DATA_DIR}/input" "${DATA_DIR}/input/kafka/done" "$DATA_DIR/documents" "$DATA_DIR/ontologies" "$DATA_DIR/readings" &
sleep 5
./bin/kafka-consumer-loop-app2 &
sleep 5

# The container ends when eidos stops running.
while ! ./bin/eidos-loop-app2 "${DATA_DIR}/input" "${DATA_DIR}/output" "${DATA_DIR}/input/done" "$DATA_DIR/documents" "$DATA_DIR/ontologies" "$DATA_DIR/readings" "${THREADS}"
do
